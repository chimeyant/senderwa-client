  ...
  <div :class="device.desktop ? `home pa-6 grey lighten-4`:`home pa-0 grey lighten-4`">
              ...
              >
                <template v-slot:default="{ active }">
                  ...
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle>{{ item.job_desc }} </v-list-item-subtitle>
                    <v-list-item-subtitle>{{ 'Porgress :'+   item.proses +" % " }} </v-list-item-subtitle>
                    <v-list-item-subtitle>{{ item.job_date }} </v-list-item-subtitle>
    ...
    <v-col cols="12">
      <v-dialog
        ...
        v-model="form.add"
        :max-width="device.desktop ? `700px` : `100%`"
        persistent
        ...
        <v-card>
          ...
          >
            <v-icon
              small
              class="mr-1 orange--text animate__animated animate__flash animate__infinite"
            >mdi-circle</v-icon> Formulir Pencetakan Laporan Harian
          </v-toolbar>
          <v-card-text class="mt-2">
            <v-col cols="12">
              <v-select
...
export default {
  ...
  data: () => ({
    ...
    jenislaporans: [
      {
        text: "Rekapitulasi Laporan Harian",
        value: "rekapitulasi-laporan-harian",
      },
      {
        text: "Rekapitulasi Laporan Bulanan",
        value: "rekapitulasi-laporan-bulanan",
      },
      {
        text: "Laporan Harian Per TA/TPP",
        value: "laporan-harian-per-orang-harian",
      },
      {
        text: "Laporan Bulanan Per TA/TPP",
        value: "laporan-harian-per-orang-bulanan",
      },
    ],
    personals: [],
  }),
    ...
    postExportXLS: async function () {
      try {
        ...
        let { data } = await this.http.get(
          "api/aktifitas/laporan-harian-export-excel",
          this.record
        );
        ...
        XLSX.utils.book_append_sheet(wb, dataxls, "Laporan Harian"); // sheetAName is name of Worksheet
        //XLSX.utils.book_append_sheet(wb, pokemonWS, "pokemons");

        // export Excel file
        XLSX.writeFile(wb, "laporan.xlsx"); // name of the file is 'book.xlsx'
      } catch (error) {
      } finally {
        this.setLoading({ dialog: false, text: "" });
      }
    },
    openFormCetak: function () {
      this.formcetak.show = true;
    },
    closeFormCetak: function () {
      this.formcetak.show = false;
    },
  },
};
</script>