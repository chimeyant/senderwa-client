            ...
            >
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  icon
                  v-on="on"
                  @click="$router.push({name:'master-program',params:{pilar_uuid: $route.params.pilar_uuid }})"
                >
                  <v-icon :color="theme.mode == 'dark' ? `white` : `black`">mdi-close-circle</v-icon>
                </v-btn>
              </template>
              <span>Tutup</span>
            </v-tooltip>
      ...
      >
        <v-card>
          ...
          >
            ...
            >mdi-circle</v-icon> Formulir Kegiatan
          </v-toolbar>
          <v-card-text class="mt-2">
            ...
            <v-col col="12">
              <v-select
                ...
                dense
                hide-details
                v-model="record.penanggung_jawab"
                :filled="record.penanggung_jawab"
                :items="opds"
                multiple
              ></v-select>
            </v-col>
            <v-col col="12">
              <v-select
                ...
                dense
                hide-details
                v-model="record.pendukung"
                :filled="record.pendukung"
                :items="opds"
                multiple
              ></v-select>
            </v-col>
            <v-col col="12">
              <v-text-field
                ...
                v-model="record.pendanaan"
                :filled="record.pendanaan"
                dense
              ></v-text-field>
            </v-col>
            ...
            <v-col cols="12">
              <v-text-field
                ...
                v-model="record.nourut"
                :filled="record.nourut"
                dense
                type="number"
              ></v-text-field>
            </v-col>
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl:
        "api/superadmin/master-data/kegiatan/" +
        this.$route.params.program_uuid,
      pagination: false,
      title: "RENAKSI (KEGIATAN)",
      subtitle: "Berikut Daftar Seluruh Kegiatan Yang Tersedia",
      showtable: true,
      breadcrumbs: [
        {
          text: "Data Master",
  ...
  methods: {
    ...
    fetchOpds: async function () {
      try {
        let { data } = await this.http.get("api/combo/opd");
        this.opds = data;
      } catch (error) {}
    },
  },
