      ...
      <v-col cols="12">
        <v-card class="animated animate__bounceIn rounded-0">
          ...
          >
            ...
            <template v-slot:item.doorstop="{ value }">
              <v-chip
                :color="value.color"
                small
              >{{ value.text }}</v-chip>
      ...
      >
        <v-card>
          ...
          >
            ...
            >mdi-circle</v-icon> Formulir Data Perlintasan
          </v-toolbar>
          <v-card-text class="mt-2">
            ...
            <v-col cols="12">
              <v-switch
                label="Tersedia palang pintu"
                :color="theme.color"
                v-model="record.doorstop"
                hide-details
              ></v-switch>
            </v-col>
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/superadmin/master-data/perlintasan-kereta-api",
      pagination: false,
      title: "MASTER PERLINTASAN",
      subtitle: "Berikut Daftar Seluruh Perlintasan Kereta Api Yang Tersedia",
      showtable: true,
      breadcrumbs: [
        {
          text: "Data Master",
  ...
  methods: {
    ...
    fetchSekolahs: async function () {
      try {
        let { data } = await this.http.get("api/combo/tingkat-sekolah");
        this.tingkats = data;
      } catch (error) {}
    },
  },
