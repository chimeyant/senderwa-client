      ...
      <v-col cols="12">
        <v-card class="animated animate__bounceIn rounded-0">
          ...
          >
            ...
            <template v-slot:item.kondisi="{ value }">
              <v-chip
                :color="value.color"
                small
              >{{ value.text }}</v-chip>
      ...
      >
        <v-card>
          ...
          >
            ...
            >mdi-circle</v-icon> Formulir Data Kendaraan Operasional
          </v-toolbar>
          <v-card-text class="mt-2">
            <v-col cols="12">
              <v-select
                ...
                dense
                hide-details
                v-model="record.jenis_kendaraan_operational_uuid"
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/superadmin/master-data/kendaraan-operasional",
      pagination: false,
      key: "id",
      title: "MASTER KENDARAAN OPERASIONAL",
      subtitle: "Berikut Daftar Seluruh Kendaraan Yang Tersedia",
      breadcrumbs: [
        {
          text: "Data Master",
          disabled: false,
  ...
  methods: {
    ...
    fetchJenisKendaraans: async function () {
      try {
        let { data } = await this.http.get("api/combo/jenis-kendaraan");
        this.jeniskendaraans = data;
      } catch (error) {}
    },
  },
};
