              ...
              >
                ...
                <v-list>
                  <v-list-item @click="openRegency(value)">
                    <v-list-item-title>
                      <v-icon :color="theme.color">mdi-map</v-icon>
                      Atur Kabupaten
                    </v-list-item-title>
                  </v-list-item>
                  <v-divider v-if="page.actions.delete || page.actions.edit "></v-divider>
                  <v-list-item
                    @click="openRegency(value)"
              ...
              >
                <template v-slot:default="{ active }">
                  ...
                  <v-list-item-content>
                    ...
                    <v-list-item-subtitle>{{ item.jmlkab }} Kabupaten</v-list-item-subtitle>
                  </v-list-item-content>
                  <v-list-item-action>
                    <v-menu
                      bottom
                    ...
                    >
                      ...
                      <v-list>
                        <v-list-item @click="openRegency(item.id)">
                          <v-list-item-title>
                            <v-icon :color="theme.color">mdi-map</v-icon>
                            Atur Kabupaten
                          </v-list-item-title>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "JML. KAB", value: "jmlkab", width: 160, align: "center" },
      {
        text: "AKSI",
        value: "id",
        width: 100,
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/superadmin/master-data/provinces",
      pagination: false,
      title: "MANAJEMEN WILAYAH",
      subtitle: "Berikut Daftar Provinsi Yang Tersedia",
      showtable: true,
      breadcrumbs: [
        {
          text: "Wilayah",
    ...
    this.fetchRecords().then(() => {
      setTimeout(() => {
        this.table.options.itemsPerPage = this.$route.params.itemsPerPage || 10;
        this.table.options.page = this.$route.params.page || 1;
      }, 100);
    });
  },
  mounted() {},
